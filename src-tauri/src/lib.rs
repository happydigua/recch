...
async fn test_connection(config: ConnectionConfig) -> Result<String, String> {
    match config.db_type.as_str() {
        "mysql" => {
            ...
            let _ = sqlx::query("SELECT 1").fetch_one(&mut conn).await.map_err(|e| e.to_string())?;
        ...
        "redis" => {
            let url = if let Some(pass) = &config.password {
                format!("redis://:{}@{}:{}/{}", pass, config.host, config.port, config.database.as_deref().unwrap_or("0"))
            } else {
                format!("redis://{}:{}/{}", config.host, config.port, config.database.as_deref().unwrap_or("0"))
            };
            
            let client = redis::Client::open(url).map_err(|e| e.to_string())?;
            let mut con = client.get_connection().map_err(|e| e.to_string())?;
            let _: String = redis::cmd("PING").query(&mut con).map_err(|e| e.to_string())?;
    ...
    match config.db_type.as_str() {
        "mysql" => {
            let mut opts = MySqlConnectOptions::new()
                .host(&config.host)
                .port(config.port);
            if let Some(user) = &config.username { opts = opts.username(user); }
            if let Some(pass) = &config.password { opts = opts.password(pass); }
        ...
        "postgresql" => {
            ...
            let mut conn = opts.connect().await.map_err(|e| e.to_string())?;
            let dbs: Vec<String> = sqlx::query_scalar("SELECT datname FROM pg_database WHERE datistemplate = false")
                .fetch_all(&mut conn)
                .await
                .map_err(|e| e.to_string())?;
            Ok(dbs)
        ...
        "redis" => {
            ...
            if let Some(pass) = &config.password {
                if !pass.is_empty() {
                    let _ : () = redis::cmd("AUTH").arg(pass).query_async(&mut con).await.map_err(|e| e.to_string())?;
                }
            }
            ...
            for i in 0..16 {
                // Select db
                let _ : () = redis::cmd("SELECT").arg(i).query_async(&mut con).await.map_err(|e| e.to_string())?;
                // Get key count
                let count: i64 = redis::cmd("DBSIZE").query_async(&mut con).await.unwrap_or(0);
...
#[tauri::command]
async fn get_tables(config: ConnectionConfig, database: Option<String>) -> Result<Vec<String>, String> {
    match config.db_type.as_str() {
        "mysql" => {
            let mut opts = MySqlConnectOptions::new()
                .host(&config.host)
        ...
        "postgresql" => {
            ...
            let mut conn = opts.connect().await.map_err(|e| e.to_string())?;
            let tables: Vec<String> = sqlx::query_scalar("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'")
                .fetch_all(&mut conn)
                .await
                .map_err(|e| e.to_string())?;
            Ok(tables)
        ...
        "redis" => {
            ...
            let _ : () = redis::cmd("SELECT").arg(db_index).query_async(&mut con).await.map_err(|e| e.to_string())?;
            
            // Get all keys (limited to 1000 for performance)
            let keys: Vec<String> = redis::cmd("KEYS").arg("*").query_async(&mut con).await.map_err(|e| e.to_string())?;
            Ok(keys)
        }
...
#[tauri::command]
async fn get_columns(config: ConnectionConfig, table: String, database: Option<String>) -> Result<Vec<ColumnDef>, String> {
    match config.db_type.as_str() {
        "mysql" => {
            let mut opts = MySqlConnectOptions::new()
                .host(&config.host)
            ...
            let q = sqlx::query_as::<_, (Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>)>(query);
            ...
            for (name_bytes, dtype_bytes, key_bytes, null_bytes, default_bytes, comment_bytes) in rows {
                let name = name_bytes.map(|b| String::from_utf8_lossy(&b).to_string()).unwrap_or_default();
                let dtype = dtype_bytes.map(|b| String::from_utf8_lossy(&b).to_string()).unwrap_or_default();
                let key_str = key_bytes.map(|b| String::from_utf8_lossy(&b).to_string()).unwrap_or_default();
                let null_str = null_bytes.map(|b| String::from_utf8_lossy(&b).to_string()).unwrap_or_default();
        ...
        "postgresql" => {
            ...
            ";
             let rows: Vec<(String, String, Option<bool>, Option<String>, Option<String>, Option<String>)> = sqlx::query_as(query)
                .bind(&table)
                .fetch_all(&mut conn)
                .await
                .map_err(|e| e.to_string())?;
        ...
        "redis" => {
            ...
            // Get key type
            let key_type: String = redis::cmd("TYPE").arg(&table).query_async(&mut con).await.unwrap_or_else(|_| "unknown".to_string());
...
#[tauri::command]
async fn alter_table(config: ConnectionConfig, table: String, operation: AlterOperation) -> Result<(), String> {
        ...
        "mysql" => {
            match operation.op_type.as_str() {
                "add" => {
                    ...
                    let comment = col.comment.as_ref().map(|c| format!("COMMENT '{}'", c.replace("'", "''"))).unwrap_or_default();
                    let null_def = if col.is_nullable == Some(false) { "NOT NULL" } else { "NULL" };
                    let default_def = col.default_value.as_ref().map(|d| format!("DEFAULT {}", d)).unwrap_or_default();
                    let pk_def = if col.is_pk { "PRIMARY KEY" } else { "" };
                    
                    format!("ALTER TABLE {} ADD COLUMN {} {} {} {} {} {}", table, col.name, col.type_name, null_def, default_def, pk_def, comment)
                },
                "modify" => {
                    let col = operation.column_def.as_ref().ok_or("Missing column definition")?;
                    let comment = col.comment.as_ref().map(|c| format!("COMMENT '{}'", c.replace("'", "''"))).unwrap_or_default();
                    ...
                    format!("ALTER TABLE {} MODIFY COLUMN {} {} {} {} {}", table, col.name, col.type_name, null_def, default_def, comment)
                },
                "drop" => {
                ...
                "rename" => {
                   ...
                   format!("ALTER TABLE {} RENAME COLUMN {} TO {}", table, col_name, new_name)
                },
                "add_index" => {
                    let idx = operation.index_def.as_ref().ok_or("Missing index definition")?;
                    let cols = idx.columns.join(", ");
                    let unique = if idx.is_unique { "UNIQUE" } else { "" };
                    format!("CREATE {} INDEX {} ON {} ({})", unique, idx.name, table, cols)
                },
                "drop_index" => {
        ...
        "postgresql" => {
             match operation.op_type.as_str() {
                "add" => {
                    ...
                    format!("ALTER TABLE {} ADD COLUMN {} {}", table, col.name, col.type_name)
                },
                "modify" => {
                    let col = operation.column_def.as_ref().ok_or("Missing column definition")?;
                    // PG: ALTER TABLE ... ALTER COLUMN ... TYPE ...
                    format!("ALTER TABLE {} ALTER COLUMN {} TYPE {}", table, col.name, col.type_name)
                },
                "drop" => {
                    let col_name = operation.column_name.as_ref().ok_or("Missing column name")?;
                    format!("ALTER TABLE {} DROP COLUMN {}", table, col_name)
    ...
    match config.db_type.as_str() {
        "mysql" => {
            ...
            let mut conn = opts.connect().await.map_err(|e| e.to_string())?;
            sqlx::query(&query).execute(&mut conn).await.map_err(|e| e.to_string())?;
        },
        "postgresql" => {
             let mut opts = PgConnectOptions::new()
                .host(&config.host)
            ...
            if operation.op_type == "add" && config.db_type == "postgresql" {
                 if let Some(col) = operation.column_def.as_ref() {
                     if let Some(comment) = &col.comment {
                         let comment_query = format!("COMMENT ON COLUMN {}.{} IS '{}'", table, col.name, comment.replace("'", "''"));
...
#[tauri::command]
async fn execute_query(config: ConnectionConfig, query: String) -> Result<Vec<HashMap<String, Value>>, String> {
    match config.db_type.as_str() {
        "mysql" => {
            let mut opts = MySqlConnectOptions::new()
                .host(&config.host)
            ...
            for row in rows {
                ...
                for col in row.columns() {
                    ...
                    let value: Value = match type_name {
                        ...
                        "TINYINT" | "SMALLINT" | "INT" | "INTEGER" | "BIGINT" | "INT4" | "INT8" | 
                        "TINYINT UNSIGNED" | "SMALLINT UNSIGNED" | "INT UNSIGNED" | "INTEGER UNSIGNED" | "BIGINT UNSIGNED" => {
                            // Try i64 first
                        ...
                        _ => {
                            // Fallback to string for TEXT, VARCHAR, DATE, TIMESTAMP, etc.
                            // Some types might fail simple generic decode (like BLOB or binary DATETIME without chrono)
                            match row.try_get::<Option<String>, _>(col.ordinal()) {
                                Ok(v) => json!(v),
                                Err(_) => {
                                    ...
                                    match row.try_get::<Option<Vec<u8>>, _>(col.ordinal()) {
                                         Ok(Some(v)) => {
                                            let hex: String = v.iter().take(16).map(|b| format!("{:02X}", b)).collect();
                    ...
                    let value: Value = match type_name {
                        ...
                        "TIMESTAMP" | "TIMESTAMPTZ" => {
                             ...
                             if let Ok(v) = row.try_get::<Option<String>, _>(col.ordinal()) {
                                 json!(v)
                             } else if let Ok(v) = row.try_get::<Option<chrono::NaiveDateTime>, _>(col.ordinal()) {
                                 json!(v.map(|d| d.to_string()))
                             } else if let Ok(v) = row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>(col.ordinal()) {
                                 json!(v.map(|d| d.to_string()))
                             } else {
                                 Value::Null
                             }
            ...
            fn redis_value_to_string(v: redis::Value) -> String {
                match v {
                    ...
                    _ => {
                        ...
                        let s: redis::RedisResult<String> = redis::FromRedisValue::from_redis_value(&v);
...
#[tauri::command]
async fn get_redis_key_value(config: ConnectionConfig, key: String, database: Option<String>) -> Result<RedisKeyInfo, String> {
    let url = format!("redis://{}:{}/", config.host, config.port);
    let client = redis::Client::open(url).map_err(|e| e.to_string())?;
    let mut con = client.get_multiplexed_async_connection().await.map_err(|e| e.to_string())?;
    ...
    let (value, length) = match key_type.as_str() {
        "string" => {
            let v: String = redis::cmd("GET").arg(&key).query_async(&mut con).await.unwrap_or_default();
            (v, None)
        },
        "list" => {
            let len: i64 = redis::cmd("LLEN").arg(&key).query_async(&mut con).await.unwrap_or(0);
            let items: Vec<String> = redis::cmd("LRANGE").arg(&key).arg(0).arg(99).query_async(&mut con).await.unwrap_or_default();
            (serde_json::to_string_pretty(&items).unwrap_or_default(), Some(len))
        },
        "set" => {
            let len: i64 = redis::cmd("SCARD").arg(&key).query_async(&mut con).await.unwrap_or(0);
            let items: Vec<String> = redis::cmd("SMEMBERS").arg(&key).query_async(&mut con).await.unwrap_or_default();
            (serde_json::to_string_pretty(&items).unwrap_or_default(), Some(len))
        },
        "zset" => {
            let len: i64 = redis::cmd("ZCARD").arg(&key).query_async(&mut con).await.unwrap_or(0);
            let items: Vec<String> = redis::cmd("ZRANGE").arg(&key).arg(0).arg(99).arg("WITHSCORES").query_async(&mut con).await.unwrap_or_default();
            (serde_json::to_string_pretty(&items).unwrap_or_default(), Some(len))
        },
        "hash" => {
            let len: i64 = redis::cmd("HLEN").arg(&key).query_async(&mut con).await.unwrap_or(0);
            let items: Vec<String> = redis::cmd("HGETALL").arg(&key).query_async(&mut con).await.unwrap_or_default();
            ...
            (serde_json::to_string_pretty(&map).unwrap_or_default(), Some(len))
        },
